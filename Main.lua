-- ============================================
-- ЧАСТЬ 1: GUI (БЕЗ GETHUI, ДЛЯ МОБИЛЫ)
-- ============================================
...
pcall(function()
    if game:GetService("CoreGui"):FindFirstChild("WhiskiUI") then
        game:GetService("CoreGui").WhiskiUI:Destroy()
    end
end)
...
local function Tween(p, props, time, style)
    local tw = TS:Create(p, TweenInfo.new(time or 0.35, style or Enum.EasingStyle.Quart, Enum.EasingDirection.Out), props)
    tw:Play()
    return tw
end
...
function CreatePage(name, buildFunction)
    ...
    content = page
    if buildFunction then buildFunction() end
    content = oldContent
    pages[name] = page
    return page
end

function SidebarButton(name, callback)
    ...
    btn.MouseButton1Click:Connect(function()
        for _, child in ipairs(sidebar:GetChildren()) do
            if child:IsA("TextButton") then
                ...
                if ind then Tween(ind, {Size = UDim2.new(0,2,0,0)}, 0.2) end
                child.TextColor3 = Theme.Sub
            end
        end
        Tween(indicator, {Size = UDim2.new(0,2,0,20)}, 0.25, Enum.EasingStyle.Back)
...
function WhiskiUI:AddLabel(text)
...
function WhiskiUI:AddButton(text, callback)
    local f = Instance.new("TextButton")
    f.Size = UDim2.new(1, 0, 0, 32)
    f.BackgroundColor3 = Theme.El
...
function WhiskiUI:AddToggle(text, default, callback)
    ...
    toggle.BackgroundColor3 = state and Theme.Accent or Theme.Stroke
...
function WhiskiUI:AddSlider(text, min, max, default, callback)
...
function WhiskiUI:AddDropdown(text, options, default, callback)
    local open = false
    ...
    sel.Size = UDim2.new(0, 70, 0, 34)
    sel.Position = UDim2.new(1, -80, 0, 0)
    ...
    holder.Size = UDim2.new(1, -16, 0, #options * 30)
    ...
    for _, opt in ipairs(options) do
        ...
        ob.MouseButton1Click:Connect(function()
            ...
            sel.Text = opt
            open = false
            Tween(f, {Size = UDim2.new(1,0,0,34)}, 0.3)
    ...
    btn.MouseButton1Click:Connect(function()
        open = not open
        if open then
            Tween(f, {Size = UDim2.new(1,0,0,34 + #options * 30 + 10)}, 0.3)
        else
            Tween(f, {Size = UDim2.new(1,0,0,34)}, 0.3)
        end
    end)
...
function WhiskiUI:AddColorPicker(text, callback)
    local hue = 0.5
    local bright = 1
    local open = false
    local f = Instance.new("Frame")
    ...
    preview.BackgroundColor3 = Color3.fromHSV(hue, 0.85, bright)
    preview.Parent = f
    Corner(preview, 6)
    ...
    picker.Size = UDim2.new(1, -24, 0, 50)
    picker.Position = UDim2.new(0, 12, 0, 40)
    picker.BackgroundTransparency = 1
    picker.ZIndex = 4
    picker.Parent = f
    local function strip(y, colorSeq)
    ...
    local function applyColor()
        local col = Color3.fromHSV(hue, 0.85, bright)
        preview.BackgroundColor3 = col
        if callback then callback(col) end
    end
    local colors = {}
    for i = 0, 6 do
        table.insert(colors, ColorSequenceKeypoint.new(i / 6, Color3.fromHSV(i / 6, 0.85, 1)))
    end
    local hueStrip, hueCursor = strip(0, ColorSequence.new(colors))
    local brightStrip, brightCursor = strip(20, ColorSequence.new(Color3.new(0,0,0), Color3.new(1,1,1)))
    brightCursor.Position = UDim2.new(1, -2, 0, 0)
    brightCursor.AnchorPoint = Vector2.new(1, 0)
    local function bind(strip, cursor, setter)
        local sliding = false
        local function update(x)
            local r = math.clamp((x - strip.AbsolutePosition.X) / math.max(strip.AbsoluteSize.X, 1), 0, 1)
            cursor.Position = UDim2.new(r, 0, 0, 0)
            setter(r)
            applyColor()
        end
    ...
    bind(hueStrip, hueCursor, function(r) hue = r end)
    bind(brightStrip, brightCursor, function(r) bright = r end)
    btn.MouseButton1Click:Connect(function()
        open = not open
        if open then
            Tween(f, {Size = UDim2.new(1,0,0,100)}, 0.3)
        else
            Tween(f, {Size = UDim2.new(1,0,0,36)}, 0.3)
        end
    end)
    return f
end
...
CreatePage("Settings", function()
    WhiskiUI:AddLabel("WhiskiUI v3.0")
    WhiskiUI:AddLabel("By: WhiskiSoul")
    WhiskiUI:AddLabel("Mobile Ready")
end)
...
print("WhiskiUI v3.0 GUI loaded")
-- ============================================
-- ЧАСТЬ 2: REMOTESPY (ПЕРЕХВАТ ВСЕХ РЕМОУТОВ)
-- ============================================
local RemoteLogs = {}
...
local function ShowRemoteModal(remote, remoteName, argsString, isFunction, response, ...)
    ...
    modal.Size = UDim2.new(0, 340, 0, 250)
    modal.Position = UDim2.new(0.5, -170, 0.5, -125)
    ...
    if isFunction and response ~= nil then
        ...
        respLabel.Position = UDim2.new(0, 10, 0, 105)
    ...
    for _, v in ipairs(game:GetDescendants()) do
        if v:IsA("RemoteEvent") then
            ...
            v.FireServer = function(self, ...)
                ...
                task.spawn(function()
                    ShowRemoteModal(self, self.Name or "Unknown", argsString, false, nil, unpack(args))
                end)
                task.wait(0.05)
                return orig(self, ...)
            end
        elseif v:IsA("RemoteFunction") then
            ...
            v.InvokeServer = function(self, ...)
                ...
                task.spawn(function()
                    ShowRemoteModal(self, self.Name or "Unknown", argsString, true, response, unpack(args))
                end)
                return response
            end
        end
...
function UpdateUI()
    for _, child in ipairs(content:GetChildren()) do
        if child:IsA("ScrollingFrame") then
            ...
            child.CanvasSize = UDim2.new(0, 0, 0, count * 55 + 20)
            ...
            for i = startIdx, count do
                ...
                if entry then
                    ...
                    btn.MouseButton1Click:Connect(function()
                        ...
                        detail.Size = UDim2.new(0, 360, 0, 250)
                        detail.Position = UDim2.new(0.5, -180, 0.5, -125)
                        detail.BackgroundColor3 = Theme.BG
                        detail.BorderSizePixel = 0
                        detail.ZIndex = 20
                        detail.Parent = main
                        Corner(detail, 12)
                        Stroke(detail, Theme.Stroke, 1, 0.5)
                        ...
                        closeBtn.Parent = detail
                        closeBtn.MouseButton1Click:Connect(function() detail:Destroy() end)
...
-- Создание страницы RemoteSpy
CreatePage("RemoteSpy", function()
...
print("WhiskiUI v3.0 RemoteSpy loaded")
